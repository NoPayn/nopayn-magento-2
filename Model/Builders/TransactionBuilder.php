...
class TransactionBuilder
{
    ...
    public function updateMailingAddress(OrderInterface $order, $method, $transaction)
    {
        ...
        $mailingAddress = $mailingAddress ?? ''; // Ensure $mailingAddress is a string
        $grandTotal = $this->configRepository->formatPrice($transaction['amount'] / 100);
        $reference = '';

        // Check if transactions array exists and is not empty
        if (!empty($transaction['transactions']) && is_array($transaction['transactions'])) {
            $currentTransaction = current($transaction['transactions']);
            // Check if payment_method_details and reference exist
            if (isset($currentTransaction['payment_method_details']['reference'])) {
                $reference = $currentTransaction['payment_method_details']['reference'];
            }
        }
    ...
    public function processRequest(OrderInterface $order, $transaction = null, $testModus = null): array
    {
        ...
        if ($transactionId && !$this->configRepository->getError($transaction)) {
            $method = $this->getMethodFromOrder($order);
            $message = __('NoPayn Order ID: %1', $transactionId);
        ...
        // No special handling for specific payment methods
    ...
    public function processing(array $transaction, OrderInterface $order, string $type): array
    {
        ...
        // No special handling for specific payment methods in non-webhook type
